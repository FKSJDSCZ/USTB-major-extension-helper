...
class MainWindowService(QObject):
	...
	def operateCourse(self, isChooseCourse: bool, **kwargs) -> None:
		self.manager_.post(
	...
	def _handleOperateCourse(self, reply: QNetworkReply) -> None:
		print(json.loads(reply.readAll().data().decode()))
	...
	def deleteAutoCourse(self) -> None:
		for row in sorted(self.model_.selectedAutoCourse, key=lambda x: x.row(), reverse=True):
	...
	def _onTimerTimeout(self) -> None:
		...
		self.operateCourse(True, kkid=autoCourseInfo["kcid"], jtxh=autoCourseInfo["jtxh"])
